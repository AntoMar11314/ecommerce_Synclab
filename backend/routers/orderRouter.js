...
orderRouter.post('/',
    ...
    expressAsyncHandler(async (req, res) => {
        if (req.body.orderItems.length === 0) {
            res.status(400).send({ messsage: 'Il carrello è vuoto' })
        } else {
            ...
            res.status(201).send({ messsage: "Nuovo ordine creato", order: createdOrder })
        }
    }));
...
orderRouter.delete('/:id', isAuth, isAdmin, expressAsyncHandler(async (req, res) => {
    ...
    if (order) {
        ...
        res.send({ message: 'Ordine eliminato', order: deletedOrder })
    } else {
        res.status(404).send({ message: 'Ordine non trovato' });
    }
}))
